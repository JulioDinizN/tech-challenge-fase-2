resource "oci_identity_policy" "oke_workloads" {
  ...
  statements = [
    ...
    "Allow any-user to use secret-family in compartment id ${var.compartment_id} where all {request.principal.type = 'workload', request.principal.namespace = '${var.secrets_provider_namespace}', request.principal.service_account = '${var.secrets_provider_service_account}', request.principal.cluster_id = '${oci_containerengine_cluster.main.id}', target.vault.id = '${oci_kms_vault.application.id}'}",
  ]
}
